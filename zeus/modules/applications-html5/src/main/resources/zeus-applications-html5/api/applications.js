...
svc.mappings().create()
	...
	.onAfterEntityInsert(function(entity, ids, context){
		if (ids !== undefined){
			producer.queue("zeus.applications.html5.created").send(""+ids);
		}
	});
svc.mappings().update()	
	.onEntityUpdate(function(entity, context){
		...
		if (entity.id !== undefined){
			producer.queue("zeus.applications.html5.updated").send(""+entity.id);
		}
	});
...
svc.mappings().remove()		
	.onAfterRemove(function(id, context){
		producer.queue("zeus.applications.html5.deleted").send(id);
	});	
